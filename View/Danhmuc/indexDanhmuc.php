...
<body>
    <div class="content" >
        ...
        <div style="margin-left: 50px; padding-bottom: 15px; padding-top: 15px; display: inline-block;">
            <h3 style="color: #000000; display: inline-block; font-size: 35px;">Danh mục</h3>
            <a href="createDanhmuc.php" class="btn btn-outline-info" style="display: inline-block; margin-left: 700px; background-color: #064928; color: white; font-size: 18px;">
        ...
        <div style="margin-top: 10px; padding-left: 50px;padding-right: 50px; display: flex; justify-content: space-between;" >
            <form method="Post" class="search">
                <input type="text" name="txtSearch" placeholder="Tìm kiếm theo tên danh mục" style="border: 1px solid #000; font-size: 16px;"value="<?php echo isset($_POST['txtSearch']) ? $_POST['txtSearch'] : $Danhmuc ?>">
                <button type="submit" name="btnSearch" id="btnSearch" class="btn btn-primary" style="background-color: #006400; ">Tìm kiếm</button>
                <br></br>
            </form>
        </div>
        ...
        <div style="padding-left: 50px;padding-right: 50px;">
            <?php 
                ...
                $result = mysqli_query($conn, 'SELECT COUNT(id) AS total FROM categories');
                ...
                while($row = mysqli_fetch_assoc($result)){
                    ...
                    echo "<td>" .$row["id"]."</td>";
                    echo "<td>" .$row["name"]."</td>";
                    echo "<td>" ."<a href = 'editDanhmuc.php?id=".$row["id"]."' class='btn btn-outline-secondary'>Sửa</a>" ." 
                    "."<a onclick='return confirm(\"Bạn có muốn xóa dữ liệu không?\")'
                    href = 'deleteDanhmuc.php?id=".$row["id"]."' class='btn btn-outline-danger'>Xóa</a>"."</td>";
                    echo "</tr>";
                }
                echo "</tbody></table>";
                }
            ...
            <div style="margin-top: 10px; display: flex; justify-content: space-between;" >
                ...
                <?php 
                    if ($current_page > 1 && $total_page > 1) {
                        echo '<a href="indexDanhmuc.php?page=' . ($current_page - 1) . '">&#171;</a> | ';
                    }
                    for ($i = 1; $i <= $total_page; $i++){
                        if ($i == $current_page){
                            ...
                            echo '<a href="indexDanhmuc.php?page='.$i.'">'.$i.'</a> |';
                        }
                    }
