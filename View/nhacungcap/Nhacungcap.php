...
<div class="content" >
    ...
    <div style="margin-left: 50px; padding-bottom: 15px; padding-top: 15px; display: inline-block;">
        <h3 style="color: #000000; display: inline-block; font-size: 35px;">Nhà cung cấp</h3>
        <a href="themNCC.php" class="btn btn-outline-info" style="display: inline-block; margin-left: 700px; background-color: #064928; color: white; font-size: 18px;">
    ...
    <div style="margin-top: 10px; padding-left: 50px;padding-right: 50px; display: flex; justify-content: space-between;" >
        <form method="Post" class="search">
            <input type="text" name="txtSearch" placeholder="Tìm kiếm theo tên nhà cung cấp" style="width: 250px; border: 1px solid #000; font-size: 16px;"value="<?php echo isset($_POST['txtSearch']) ? $_POST['txtSearch'] : $TenKH ?>">
    ...
    <div style="padding-left: 50px;padding-right: 50px;">
        <?php 
            ...
            $result = mysqli_query($conn, 'SELECT COUNT(id) AS total FROM supplier');
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                echo "<thead>";
                echo "<th style='width: 5%; text-align: center;'>STT</th>";
                echo "<th style='width: 20%; text-align: center;'>Tên NCC</th>";
                echo "<th style='width: 10%; text-align: center;'>Số điện thoại</th>";
                echo "<th style='width: 30%; text-align: center;'>Địa chỉ</th>";
                echo "<th style='width: 20%; text-align: center;'>Mô tả</th>";
                echo "<th style='width: 10%; text-align: center;'>Thao Tác</th>";
                echo "</thead>";
                echo "<tbody>";
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    echo "<td style='text-align: center;'>"."<a href = 'suaNCC.php?id=".$row["id"]."' class='btn btn-outline-secondary'>Sửa</a>"."</td>";
                    echo "</tr>";
                }
                echo "</tbody></table>";
            } else {
        ...
        <div style="margin-top: 10px; display: flex; justify-content: space-between;" >
            ...
            <?php 
                if ($current_page > 1 && $total_page > 1) {
                    echo '<a href="Nhacungcap.php?page=' . ($current_page - 1) . '">&#171;</a> | ';
                }
                for ($i = 1; $i <= $total_page; $i++){
                    if ($i == $current_page){
                        ...
                        echo '<a href="Nhacungcap.php?page='.$i.'">'.$i.'</a> |';
                    }
                }
