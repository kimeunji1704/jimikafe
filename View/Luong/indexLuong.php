...
<div class="content" >
    ...
    <div style="margin-left: 50px; padding-bottom: 15px; padding-top: 15px; display: inline-block;">
        <h3 style="color: #000000; display: inline-block; font-size: 35px;">Danh sách lương nhân viên</h3>
        <a href="createLuong.php" class="btn btn-outline-info" style="display: inline-block; margin-left: 550px; background-color: #064928; color: white; font-size: 18px;">
    ...
    <div style="margin-top: 10px; padding-left: 50px;padding-right: 50px; display: flex; justify-content: space-between;" >
        <form method="Post" class="search">
            <input type="text" name="txtSearch" placeholder="Tìm kiếm theo tên nhân viên" style="border: 1px solid #000; font-size: 16px; width: 250px;"value="<?php echo isset($_POST['txtSearch']) ? $_POST['txtSearch'] : $TenKH ?>">
    ...
    <div style="padding-left: 50px;padding-right: 50px;">
        <?php 
            ...
            $result = mysqli_query($conn, 'SELECT COUNT(id) AS total FROM salaries');
            ...
            if (isset($_POST['txtSearch'])) {
                ...
                $query = "SELECT salaries.id, employees.name, salaries.date, salaries.salary, salaries.reward, salaries.fines, salaries.workhours, salaries.total
                          FROM salaries
                          JOIN employees ON salaries.employee_id = employees.id
                          WHERE employees.name LIKE N'%".$KeyWord."%' 
                          LIMIT $start, $limit";
            } else {
                $query = "SELECT salaries.id, employees.name, salaries.date, salaries.salary, salaries.reward, salaries.fines, salaries.workhours, salaries.total
                          FROM salaries
            ...
            if(mysqli_num_rows($result)>0){
                ...
                echo "<th style='width: 15%;'>Tên nhân viên</th>";
                echo "<th style='width: 10%;'>Ngày</th>";
                echo "<th style='width: 15%;'>Lương cơ bản</th>";
                echo "<th style='width: 10%;'>Thưởng</th>";
                echo "<th style='width: 10%;'>Khấu trừ</th>";
                echo "<th style='width: 10%;'>Số giờ</th>";
                echo "<th style='width: 15%;'>Thực lĩnh</th>";
                echo "</thead>";
                echo "<tbody>";
            while($row = mysqli_fetch_assoc($result)){
                ...
                echo "<td>" . number_format($row["reward"]) . "</td>";
                echo "<td>" . number_format($row["fines"]) . "</td>";
                echo "<td>" . number_format($row["workhours"], 1) . "</td>";
    ...
    <div style="margin-top: 10px; display: flex; justify-content: space-between;" >
            ...
            <?php 
                if ($current_page > 1 && $total_page > 1) {
                    echo '<a href="indexLuong.php?page=' . ($current_page - 1) . '">&#171;</a> | ';
                }
                for ($i = 1; $i <= $total_page; $i++){
                    if ($i == $current_page){
                        ...
                        echo '<a href="indexLuong.php?page='.$i.'">'.$i.'</a> |';
                    }
                }
