...
<div class="content" >
    ...
    <div style="margin-left: 50px; padding-bottom: 15px; padding-top: 15px; display: inline-block;">
        <h3 style="color: #000000; display: inline-block; font-size: 35px;">Danh sách nhập kho</h3>
        <a href="createNhapkho.php" class="btn btn-outline-info" style="display: inline-block; margin-left:600px; background-color: #064928; color: white; font-size: 18px;">
    ...
    <div style="margin-top: 10px; padding-left: 50px;padding-right: 50px; display: flex; justify-content: space-between;" >
        <form method="Post" class="search">
            <input type="text" name="txtSearch" style="border: 1px solid #000; font-size: 16px; width: 350px;" placeholder = "Tìm kiếm theo tên nhà cung cấp hoặc tên nguyên liệu" value="<?php echo isset($_POST['txtSearch']) ? $_POST['txtSearch'] : $TenKH ?>">
    ...
    <div style="padding-left: 50px;padding-right: 50px;">
        <?php 
            ...
            $result = mysqli_query($conn, 'SELECT COUNT(id) AS total FROM import');
            ...
            if (isset($_POST['txtSearch'])) {
                ...
                $query = "SELECT import.id, supplier.supplier_name, ingredient.name, import.quantity, import.date
                          FROM import
                          JOIN supplier ON import.supplier_id = supplier.id 
                          JOIN ingredient ON import.ingredient_id = ingredient.id
                          WHERE ingredient.name LIKE N'%" . $KeyWord . "%' OR supplier.name LIKE N'%" . $KeyWord . "%'
                          LIMIT $start, $limit";
            } else {
                $query = "SELECT import.id, supplier.supplier_name, ingredient.name, import.quantity, import.date
                          FROM import
            ...
            if(mysqli_num_rows($result)>0){
                ...
                echo "<thead>";
                echo "<th style='width: 15%; text-align: center;'> Mã phiếu NK</th> ";
                echo "<th style='width: 25%; text-align: center;'> Tên nhà cung cấp</th>";
                echo "<th style='width: 25%; text-align: center;'> Tên nguyên liệu</th>";
                echo "<th style='width: 20%; text-align: center;'> Số lượng</th>";
...
<div style="margin-top: 10px; display: flex; justify-content: space-between;" >
            ...
            <?php 
                if ($current_page > 1 && $total_page > 1) {
                    echo '<a href="Nhapkho.php?page=' . ($current_page - 1) . '">&#171;</a> | ';
                }
                for ($i = 1; $i <= $total_page; $i++){
                    if ($i == $current_page){
                        ...
                        echo '<a href="Nhapkho.php?page='.$i.'">'.$i.'</a> |';
                    }
                }
