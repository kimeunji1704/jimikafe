...
<div class="content" >
    ...
    <div style="margin-left: 50px; padding-bottom: 15px; padding-top: 15px; display: inline-block;">
        <h3 style="color: #000000; display: inline-block; font-size: 35px;">Danh sách sản phẩm</h3>
        <a href="createMathang.php" class="btn btn-outline-info" style="display: inline-block; margin-left: 600px; background-color: #064928; color: white; font-size: 18px;">
    ...
    <div style="margin-top: 10px; padding-left: 50px;padding-right: 50px; display: flex; justify-content: space-between;" >
        <form method="Post" class="search">
            <input type="text" name="txtSearch" placeholder="Tìm kiếm theo danh mục hoặc tên sản phẩm" style="width: 350px; border: 1px solid #000; font-size: 17px;"value="<?php echo isset($_POST['txtSearch']) ? $_POST['txtSearch'] : $TenKH ?>">
    ...
    <div style="padding-left: 50px;padding-right: 50px;">
        <?php 
            ...
            if (isset($_POST['txtSearch'])) {
                ...
                $query = "SELECT product.id, categories.name, product.product_name, product.unit_price  
                          FROM product 
                          JOIN categories ON product.category_id = categories.id
                          WHERE categories.name LIKE N'%" . $KeyWord . "%' OR product.product_name LIKE N'%" . $KeyWord . "%' 
                          LIMIT $start, $limit";
            } else {
                $query = "SELECT product.id, categories.name, product.product_name, product.unit_price  
                          FROM product 
            ...
            while($row = mysqli_fetch_assoc($result)){
                ...
                echo "<td>" ."<a href = 'editMathang.php?id=".$row["id"]."' class='btn btn-outline-secondary'>Sửa</a>" ." 
				"."<a onclick='return confirm(\"Bạn có muốn xóa dữ liệu không?\")'
                 href = 'deleteMathang.php?id=".$row["id"]."' class='btn btn-outline-danger'>Xóa</a>"."</td>";
                echo "</tr>";
            }
            echo "</tbody></table>";
            }
...
<div style="margin-top: 10px; display: flex; justify-content: space-between;" >
            ...
            <?php 
                if ($current_page > 1 && $total_page > 1) {
                    echo '<a href="indexMathang.php?page=' . ($current_page - 1) . '">&#171;</a> | ';
                }
                for ($i = 1; $i <= $total_page; $i++){
                    if ($i == $current_page){
                        ...
                        echo '<a href="indexMathang.php?page='.$i.'">'.$i.'</a> |';
                    }
                }
